...
# So what does this do? It creates a griffin initramfs (ramdisk) from
# the files provided in ./root/. It automatically replaces the mount
# points in init.griffin.rc, based on which argument is passed to the
...
function display_help {
    ...
    echo -e "This is a combination script, used for building ramdisks for use with the LePan TC970. It covers"
    echo -e "use with standard emmc, alternate emmc, and SD card partition setups. It uses the ./root/ folder"
    echo -e "for its input. The init file, \033[1minit.griffin.rc\033[0m will automatically be modified to contain the"
...
## update init.griffin.rc with proper partition names
